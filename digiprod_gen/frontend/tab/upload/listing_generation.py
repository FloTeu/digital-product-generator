...
def listing_generation(st_tab_ig: DeltaGenerator):
    ...
    with st_tab_ig, st.spinner('Listing generation...'):
        ...
        keywords = keyword_extractor.generate(text=keyword_text)
        keywords = ",".join([k for k in keywords.split(",") if k.lower().strip() not in MBA_BANNED_WORDS])
        ts_start = time.time()
        # bullet generation
        predicted_bullets: List[str] = extract_list_from_output(product_text_gen_bullet.generate(keywords=keywords))
        session_state.upload_data.predicted_bullets = remove_banned_words_from_list(predicted_bullets, MBA_BANNED_WORDS)
        # title generation
        predicted_titles: List[str] = extract_list_from_output(product_text_gen_title.generate(keywords=keywords))
        session_state.upload_data.predicted_titles = remove_banned_words_from_list(predicted_titles, MBA_BANNED_WORDS)
        # brand generation
        predicted_brands: List[str] = extract_list_from_output(product_text_gen_brand.generate(keywords=keywords))
