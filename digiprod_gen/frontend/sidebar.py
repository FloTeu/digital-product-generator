...
from digiprod_gen.frontend.ai_agent import run_ai_agent_product_generation
...
def search_mba_overview_input():
    ...
    st.selectbox("MBA Marketplace",
                 options=[MBAMarketplaceDomain.COM, MBAMarketplaceDomain.DE, MBAMarketplaceDomain.FR],
                 # , MBAMarketplaceDomain.ES, MBAMarketplaceDomain.IT, MBAMarketplaceDomain.JP],
...
def crawling_mba_details_input(mba_products, tab_ig: DeltaGenerator):
    ...
    st.multiselect("Select Designs for prompt generation:", [i + 1 for i in range(len(mba_products))],
                   key='selected_designs')
    st.button("Start Crawling Details", on_click=crawl_details_update_overview_page, args=(tab_ig,),
              key="button_crawl_detail")
